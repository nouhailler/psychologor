...
| Field | Comment ce domaine s'est constitué comme spécialisation distincte de la psychologie |
...
- `Field` (CHANGELOG 1.7.0) ajoute un troisième axe de classification, encore distinct des deux précédents : `School` répond à « quel mouvement historique, et quand ? », `Approach` répond à « quel angle explicatif ? », `Field` répond à « quel domaine professionnel et académique, sur quel terrain ? ». Certains noms se recoupent volontairement (« Psychologie cognitive » existe à la fois comme `Field` et comme `Approach`) sans être redondants : le champ répond à une question différente, avec ses propres débouchés (`applications`) et ses propres questions de recherche (`keyQuestions`) plutôt que des forces/limites à critiquer.
- Les cinq disciplines voisines et souvent confondues — psychologie, psychiatrie, neurosciences, psychanalyse, psychothérapie (CHANGELOG 1.8.0) — sont, comme les notions de lecture critique (1.4.0), des entrées `Concept` ordinaires plutôt qu'une nouvelle entité : le besoin ici est un contenu comparatif dans le glossaire, pas une nouvelle forme de fiche. Le concept `psychanalyse-pratique` porte volontairement un `id` distinct de la `Theory` `psychanalyse` déjà existante (le modèle freudien de l'esprit) : même terme, deux angles différents (la théorie vs. la profession/pratique), chacun renvoyant explicitement à l'autre dans son texte plutôt que de les fusionner ou de les laisser se contredire silencieusement.
